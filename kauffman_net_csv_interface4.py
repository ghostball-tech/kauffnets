...
Iterating over grid pairs of (p,k)
large numerical grid search
...
# make a square grid over the two
N_grid = 10
K = np.linspace(0.3,8.,N_grid) #
P = np.linspace(0.01,0.99,N_grid)
...
def kaufNetABM(N,x_0,K,p,T):
    ...
    mapList = []
    #variables for measuring the p-value generated from this specific s
    totalValues = 0
    sumTotal = 0
    for agent in range(N):
        agentMap = {}
...
for k in K:
    for p in P:
        for iter in range(nIter):
            ...
            p_samp, k_samp, stateMean, hamDist = kaufNetABM(N,x_0,k,p,T)
...
#output_df.to_csv('C:\dev\ABMoutput_examples\kauffman_abm_bigsim.csv',index = False)
