...
--ver 5 : import parameter values from "observed" data
...
exper_data = pd.read_csv("C:\\dev\\ABMoutput_examples\\kauffman_simfield_example1.csv")
# kauffman_simfield_example1.csv
# Columns: p, k, state_mean, hamming_dist
#E[k] = 4
# Rows 0:29 E[p] = 0.05
# Rows 30:59 E[[p] = 0.2
# Rows 60:89 E[p] = 0.4
...
#%%
data_index = np.arange(0,90)
P = np.array(exper_data.p[data_index])
K = np.array(exper_data.k[data_index])
...
for exper in range(len(data_index)):
    k= K[exper]
    p = P[exper]
...
output_df.to_csv('C:\dev\ABMoutput_examples\kauffman_abm_example3.csv',index = False)
#example3: uses simfield_example1 as input
#example 4 uses simfield_example2 as input, fixed network topology, seed = 97 MakeGraph()
